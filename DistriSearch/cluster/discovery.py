"""
DistriSearch Cluster - Servicio de Descubrimiento Multicast
...
class MulticastDiscovery:
    ...
    - Detección automática de nodos nuevos
    - Timeout para nodos perdidos
    ...
    def __init__(
        ...
        Args:
            ...
            port: Puerto del servicio HTTP del nodo
            ip_address: Dirección IP del nodo
            on_node_discovered: Callback cuando se descubre un nuevo nodo
            on_node_lost: Callback cuando se pierde un nodo
        """
    ...
    async def _process_message(self, data: bytes):
        """Procesa mensaje multicast recibido"""
        try:
    ...
    async def _handle_announce(self, message: Dict):
        ...
        # Ignorar nuestro propio anuncio
        ...
        if is_new_node:
            ...
            logger.debug(f"Heartbeat de nodo: {discovered_node_id}")
    
    async def _handle_query(self, message: Dict):
        """Responde a query de descubrimiento"""
        response = json.dumps({
            "type": "node_response",
            "node_id": self.node_id,
            "ip_address": self.ip_address,
        ...
        logger.debug("Respuesta enviada a query")
    ...
    async def _timeout_check_loop(self):
        """Verifica timeouts de nodos descubiertos"""
        while self.running:
            try:
                now = datetime.utcnow()
                lost_nodes = []
                ...
                for node_id, info in list(self.discovered_nodes.items()):
                    ...
                    if time_since_last_seen > self.discovery_timeout:
                        ...
                        logger.warning(f"Nodo perdido (timeout): {node_id}")
    ...
    async def query_nodes(self):
        """Envía query para descubrir nodos"""
        try:
            message = json.dumps({
                "type": "node_query",
                "node_id": self.node_id,
            ...
            logger.info("Query enviado para descubrir nodos")
...
_multicast_service: Optional[MulticastDiscovery] = None
...
async def get_multicast_service(
    ...
    """Obtiene o crea servicio de multicast discovery (singleton)"""
