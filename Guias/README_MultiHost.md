...
1. Iniciar Swarm en VM A (manager)
...
1. Etiquetar nodos y desplegar stack
...
## Anexos: comandos útiles (Windows PowerShell)

Comandos rápidos para cuando uses dos equipos y necesites diagnosticar o preparar el entorno.

### Abrir puertos de Swarm en Windows (en ambos equipos)

```powershell
# Gestión Swarm
netsh advfirewall firewall add rule name="Docker Swarm 2377 TCP" dir=in action=allow protocol=TCP localport=2377
# Descubrimiento
netsh advfirewall firewall add rule name="Docker Swarm 7946 TCP" dir=in action=allow protocol=TCP localport=7946
netsh advfirewall firewall add rule name="Docker Swarm 7946 UDP" dir=in action=allow protocol=UDP localport=7946
# Overlay VXLAN
netsh advfirewall firewall add rule name="Docker Swarm 4789 UDP" dir=in action=allow protocol=UDP localport=4789
```

### Inicializar Swarm y obtener tokens (en el manager)

```powershell
# Usa tu IP LAN (no 127.0.0.1)
docker swarm init --advertise-addr 192.168.1.X

# Mostrar el comando para unir workers
docker swarm join-token worker

# Solo el token (útil para copiar/pegar)
docker swarm join-token -q worker

# También token de manager (opcional)
docker swarm join-token manager
```

### Unir el worker al cluster (en el worker)

```powershell
# Prueba conectividad primero
Test-NetConnection 192.168.1.X -Port 2377

# Une el nodo worker (reemplaza IP y token)
docker swarm join --token <TOKEN_WORKER> 192.168.1.X:2377
```

### Etiquetar nodos para colocar servicios por rol (en el manager)

```powershell
# Ver nodos
docker node ls

# Etiquetas (ajusta los nombres reales)
docker node update --label-add role=backend NOMBRE_NODO_PC_A
docker node update --label-add role=frontend NOMBRE_NODO_PC_B

# Verificar etiquetas
docker node inspect --format "{{.Description.Hostname}} => {{.Spec.Labels}}" NOMBRE_NODO_PC_A
docker node inspect --format "{{.Description.Hostname}} => {{.Spec.Labels}}" NOMBRE_NODO_PC_B
```

### Distribuir imágenes sin registry (exportar/importar)

```powershell
# En el manager: construir y exportar la imagen del frontend
docker build -t distrisearch/frontend:latest .\DistriSearch\frontend
docker save -o frontend.tar distrisearch/frontend:latest

# Copia frontend.tar al worker y allí importa
docker load -i .\frontend.tar
```

### Desplegar y verificar servicios (en el manager)
...
docker service logs distrisearch_backend --since 5m
docker service logs distrisearch_frontend --since 5m
```
...
# Ping/TCP test hacia el manager
Test-NetConnection 192.168.1.X -Port 2377
```
...
# Salir del Swarm (en worker)
docker swarm leave

# En manager (con forzado si es necesario)
docker swarm leave --force

# Imágenes y volúmenes (¡cuidado!)
docker image ls
docker volume ls
```
